    ...
    return (
        <div className="card" style={{ marginBottom: '1.5rem', border: '1px solid var(--color-primary)' }}>
            <div className="flex justify-between items-center" style={{ marginBottom: '1rem' }}>
                ...
                <button onClick={onClose} style={{ fontSize: '0.9rem' }}>{t('categories.close')}</button>
            </div>
            ...
            <form onSubmit={addCategory} style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
                <input
                    ...
                    style={{ flex: 1 }}
                />
                <button type="submit" className="primary">{t('categories.add')}</button>
            </form>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        ...
                        {editingId === cat.id ? (
                            <div className="flex gap-2" style={{ flex: 1 }}>
                                <input
                                    ...
                                    autoFocus
                                />
                                <button onClick={() => updateCategory(cat.id)} className="primary" style={{ padding: '0.25rem 0.5rem', fontSize: '0.8rem' }}>{t('categories.save')}</button>
                                <button onClick={() => setEditingId(null)} style={{ padding: '0.25rem 0.5rem', fontSize: '0.8rem' }}>{t('categories.cancel')}</button>
                            </div>
                        ) : (
                            <>
                                <span>{cat.name}</span>
                ...
                {categories.length === 0 && <div style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>{t('categories.noCategories')}</div>}
            </div>
        </div>
    )
}
